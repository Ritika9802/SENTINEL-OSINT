...
def render_hash_tool():
    ...
    with tab1:
        ...
        if input_mode == "Upload File":
            ...
            with st.form("hash_text_form", clear_on_submit=False):
                ...
                generate_hash = st.form_submit_button("Generate Hash", type="primary", use_container_width=True)
            if generate_hash:
                if text_input:
    ...
    with tab2:
        ...
        with st.form("hash_integrity_form", clear_on_submit=False):
            ...
            compare_hashes = st.form_submit_button("Compare Hashes", type="primary", use_container_width=True)
    ...
    with tab3:
        ...
        with st.form("hash_lookup_form", clear_on_submit=False):
            vt_hash_input = st.text_input(
                "File Hash (MD5 / SHA-1 / SHA-256)",
                placeholder="e.g. 44d88612fea8a8f36de82e1278abb02f  or a SHA-256 hash...",
                key="vt_hash_input"
            )
            check_hash = st.form_submit_button("Check File Hash", type="primary", use_container_width=True)
                ...
                with st.spinner("Querying VirusTotal..."):
                    try:
                        ...
                        if resp.status_code == 401:
                            ...
                            st.info(" Hash not found in VirusTotal database. The file may never have been scanned.")
                        elif resp.status_code == 429:
                            st.warning(" VirusTotal rate limit exceeded. Please wait and try again.")
