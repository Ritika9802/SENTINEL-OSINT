...
def _first_available(meta: dict, keys: list[str]) -> str:
    for key in keys:
        value = meta.get(key)
...
def _file_type_label(filename: str, file_type: str) -> str:
    extension_mimes = {
        "jpg": "image/jpeg",
        "jpeg": "image/jpeg",
        "png": "image/png",
...
def _build_metadata_summary(meta: dict, filename: str, file_type: str, data: bytes) -> dict:
    width = _first_available(meta, ["Width_px"])
    height = _first_available(meta, ["Height_px"])
    resolution = (
        f"{width} x {height} px"
        if width != "Not available" and height != "Not available"
        else _first_available(meta, ["Size"])
    )

    return {
        "File Name": filename or "Not available",
        "File Type (MIME Type)": _file_type_label(filename, file_type),
        "File Size": f"{len(data):,} bytes",
        "SHA-256 Hash": hashlib.sha256(data).hexdigest(),
        "Image Resolution (Width x Height)": resolution,
        "Color Depth": _first_available(meta, ["ColorDepth", "Mode", "XMP_ColorMode"]),
        "Camera Make": _first_available(meta, ["Make", "ExifIFD.Make", "XMP_LensMake"]),
        "Camera Model": _first_available(meta, ["Model", "ExifIFD.Model", "XMP_LensModel"]),
        "Date Taken": _first_available(meta, [
            "DateTimeOriginal", "ExifIFD.DateTimeOriginal", "DateTimeDigitized",
            "XMP_DateCreated", "IPTC_DateCreated", "Created", "CreationDate"
        ]),
        "Date Modified": _first_available(meta, [
            "DateTime", "ModifyDate", "ExifIFD.DateTime", "XMP_ModifyDate",
            "XMP_MetadataDate", "Modified", "ModDate"
        ]),
        "GPS Latitude": _first_available(meta, [
            "GPS.LatitudeDecimal", "GPS.GPSLatitude", "GPSIFD.GPSLatitude", "Latitude"
        ]),
        "GPS Longitude": _first_available(meta, [
            "GPS.LongitudeDecimal", "GPS.GPSLongitude", "GPSIFD.GPSLongitude", "Longitude"
        ]),
        "Software Used": _first_available(meta, [
            "Software", "Creator", "Producer", "XMP_CreatorTool",
            "IPTC_OriginatingProgram", "Application", "Encoder"
        ]),
        "Author": _first_available(meta, ["Author", "XMP_Creator", "IPTC_Byline", "creator"]),
        "Last Modified By": _first_available(meta, ["Last Modified By", "lastModifiedBy", "LastModifiedBy"]),
        "Orientation": _first_available(meta, ["Orientation", "ExifIFD.Orientation"]),
    }


def _render_metadata_table(summary: dict, meta: dict):
    rows_data = []
    seen_fields = set()
    ...
    for field, value in summary.items():
        if value == "Not available":
            continue
        rows_data.append((field, value))
        seen_fields.add(field.lower().replace(" ", "").replace("-", "").replace("_", ""))

    for field, value in sorted(meta.items()):
        if value in (None, "", "Not available"):
            continue
        normalized_field = field.lower().replace(" ", "").replace("-", "").replace("_", "")
    ...
    if not rows:
        st.info("No metadata fields were available for this file.")
        return

    st.markdown(f"""
    <table class="meta-table meta-summary-table">
        <thead><tr><th>Metadata Information</th><th>Value</th></tr></thead>
        <tbody>{rows}</tbody>
    </table>
                ...
                for tag_id, value in exif_data.items():
                    ...
                    if tag == "GPSInfo" and isinstance(value, dict):
                        ...
                        try:
                            ...
                            meta["GPS.MapsLink"] = f"https://maps.google.com/xq={lat},{lon}"
                        except Exception:
                            pass
                    else:
                        str_val = str(value)
...
def extract_docx_metadata(data: bytes) -> dict:
    try:
        ...
        def _prop(name: str) -> str:
            return str(getattr(cp, name, "") or "")

        meta = {
            "Author":             _prop("author"),
            "Last Modified By":   _prop("last_modified_by"),
...
def render_metadata_inspector():
    ...
    if uploaded is not None:
        ...
        summary = _build_metadata_summary(meta, uploaded.name, file_type, data)

        st.markdown(f"**Metadata for {file_type} file:**")
        _render_metadata_table(summary, meta)

        st.download_button(
            label=" Download Metadata Report (JSON)",
            data=json.dumps(
                {
                    "summary": {k: v for k, v in summary.items() if v != "Not available"},
                    "full_metadata": {k: v for k, v in meta.items() if v not in (None, "", "Not available")},
                },
                indent=2
            ),
