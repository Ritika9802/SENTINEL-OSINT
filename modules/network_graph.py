...
def build_graph(domain: str, include_subdomains: bool,
    ...
    for ip in vt_ips:
        ...
        if include_threat:
            ...
            if score > 0:
                ...
                tag_id = f"abuse_{ip}"
                label = f" Abuse\n{score}%" if kind == "threat" else f"OK Clean\n{score}%"
    ...
    if include_threat and VT_API_KEY:
        if vt_mal > 0:
            ...
            _node(net, clean_id, "OK VT Clean", "clean",
...
def render_network_graph(domain: str):
    ...
    st.markdown(
        "<div style='background:#EAF4FF !important;border:1.5px solid #B7D3F2;"
        "border-left:4px solid #0078D4;border-radius:8px;padding:10px 16px;"
        "color:#000000 !important;font-size:0.92rem;font-weight:600;"
    ...
    st.markdown("""
    <style>
      .ng-legend-wrap span { color: #000000 !important; }
    </style>
    <div class='ng-legend-wrap' style='display:flex;flex-wrap:wrap;gap:10px;margin:8px 0 12px;font-size:0.8rem;'>
      <span style='background:#0064B4;color:#fff !important;padding:3px 10px;border-radius:12px;'>Target</span>
      <span style='background:#E8F0FB;border:1px solid #0064B4;padding:3px 10px;border-radius:12px;'>Subdomain</span>
      <span style='background:#FFF4E6;border:1px solid #E87B00;padding:3px 10px;border-radius:12px;'>IP Address</span>
      <span style='background:#F5F8FF;border:1px solid #9AAFC8;padding:3px 10px;border-radius:12px;'>Open Port</span>
      <span style='background:#FDECEA;border:1px solid #C0392B;padding:3px 10px;border-radius:12px;'>Threat</span>
      <span style='background:#EAF5F0;border:1px solid #1A7A52;padding:3px 10px;border-radius:12px;'>Clean</span>
      <span style='background:#F5F0FF;border:1px solid #7B68EE;padding:3px 10px;border-radius:12px;'>Geo</span>
    </div>
    """, unsafe_allow_html=True)
