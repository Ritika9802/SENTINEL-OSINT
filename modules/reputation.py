...
def fetch_vt_domain(domain):
    if not VT_API_KEY:
        st.warning(" VirusTotal API key not configured")
        return None
    r = requests.get(
    ...
    if r.status_code == 403: st.warning(" VirusTotal: Access forbidden for this API key or endpoint"); return None
    if r.status_code == 404: st.info(" VirusTotal: No reputation record found for this domain"); return None
    if r.status_code == 429: st.warning(" VirusTotal: Rate limit exceeded"); return None
...
def fetch_vt_ip(ip):
    ...
    if r.status_code == 401: st.error(" VirusTotal: Invalid API key"); return None
    if r.status_code == 403: st.warning(" VirusTotal: Access forbidden for this API key or endpoint"); return None
    if r.status_code == 404: st.info(" VirusTotal: No reputation record found for this IP"); return None
    if r.status_code == 429: st.warning(" VirusTotal: Rate limit exceeded"); return None
    if r.status_code != 200: raise requests.exceptions.RequestException(f"Status {r.status_code}")
    return r.json()
...
def fetch_shodan_ip(ip):
    if not SHODAN_API_KEY:
        st.warning(" Shodan API key not configured")
        return None
    r = requests.get(
    ...
    if r.status_code == 403: st.warning(" Shodan: Access forbidden for this API key or plan"); return None
    if r.status_code == 404: st.info(" Shodan: No info for this IP"); return None
    if r.status_code == 429: st.warning(" Shodan: Rate limit exceeded"); return None
    if r.status_code != 200: raise requests.exceptions.RequestException(f"Status {r.status_code}")
    return r.json()
...
def fetch_abuseipdb(ip):
    if not ABUSEIPDB_API_KEY:
        st.warning(" AbuseIPDB API key not configured")
        return None
    r = requests.get(
    ...
    if r.status_code == 429: st.warning(" AbuseIPDB: Rate limit exceeded"); return None
    if r.status_code != 200: raise requests.exceptions.RequestException(f"Status {r.status_code}")
    return r.json()
    ...
    if ABUSEIPDB_API_KEY:
        ...
        if ab and "data" in ab:
            ...
            if conf > 75:
                ...
                st.warning(" MODERATE RISK - reported for abuse")
