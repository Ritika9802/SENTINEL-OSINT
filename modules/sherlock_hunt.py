    ...
    if st.session_state.sherlock_scanning:
        ...
        try:
            proc = subprocess.Popen(
                [sys.executable, "-m", "sherlock_project", username, "--print-found", "--no-color", "--timeout", "30"],
            ...
            for line in proc.stdout:
                if st.session_state.sherlock_stop_requested:
                    proc.terminate()
                    proc.wait(timeout=3)
                    status_container.warning(" Scan stopped by user")
                    break
        ...
        status_container.info(" No accounts found for this username")
