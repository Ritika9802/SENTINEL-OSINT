...
def render_typosquatting(domain: str, limit):
    ...
    if dnstwist_results:
        ...
        if len(dnstwist_results) > lim:
            st.info(f" Showing {lim} of {len(dnstwist_results)} variants")
    else:
        ...
        if typos:
            ...
            for t in typos[:lim]:
                st.markdown(f"<div class='engine'>- {t}</div>",
                            unsafe_allow_html=True)
            if len(typos) > lim:
                st.info(f" Showing {lim} of {len(typos)} variants")
        else:
            st.markdown(
                ...
                " No typo variants could be generated for this domain.</div>",
                unsafe_allow_html=True
            )
